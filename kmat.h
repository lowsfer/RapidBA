...
T* badPtr(){ return reinterpret_cast<T*>(1u); }

template <typename T>
struct NullArray1D
{
    template<typename Index> __device__ __host__ __forceinline__
    T& operator[](Index i) { return *badPtr<T>(); }
    template<typename Index> __device__ __host__ __forceinline__
    const T& operator[](Index i) const { return *badPtr<T>(); }
};
template <typename T>
struct NullArray2D
{
    template<typename Index> __device__ __host__ __forceinline__
    NullArray1D<T>& operator[](Index i) { return *(NullArray1D<T>*)1u;}
    template<typename Index> __device__ __host__ __forceinline__
    const NullArray1D<T>& operator[](Index i) const { return *(NullArray1D<T>*)1u;}
};
...
//        static_assert((SrcMatType::rows() <= rows() && SrcMatType::cols() <= cols()), "fatal error"); // @fixme: enable this when if constexpr is available in cuda
    ...
    template <typename Index>
    __device__ __host__ __forceinline__
    kmat<T, 1, Cols> row(Index i) const{
        return block<1, Cols>(i, 0);
    }
    template <typename Index>
    __device__ __host__ __forceinline__
    kmat<T, Rows, 1> col(Index i) const{
        return block<Rows, 1>(0, i);
    }
...
#pragma unroll
    ...
    template <bool Enabler = true, typename = std::enable_if_t<Enabler && Rows == 1 && Cols == 1>>
    __device__ __host__ __forceinline__
    T scalar() const {
        return (*this)[0];
    }
    ...
    symkmat(const kmat<T, Rows, Rows>& src){
        for (unsigned i = 0; i < Rows; i++){
            for (unsigned j = i; j < Rows; j++){
				// asymmetric values are found due to numeric issues, maybe due to V matrix inversion
#ifndef NDEBUG
                const bool ok = (i == j || std::abs(src(i, j) - src(j, i)) < 0.1f || std::abs(src(i, j) - src(j, i)) / std::abs(src(i, j)) < 0.1f || (!std::isfinite(src(i, j)) && !std::isfinite(src(j, i))));
				if (!ok) {
					printf("src is not symmetric: (%u, %u) - {%f, %f}\n", i, j, src(i,j), src(j,i));
				}
#endif
                (*this)(i, j) = src(i, j);
            }
